...
<div class="container mx-auto mt-8 px-4 max-w-4xl">
    <div class="bg-white shadow-xl rounded-lg overflow-hidden border border-gray-200">
        <div class="bg-gray-800 px-6 py-4">
            <h2 class="text-xl font-bold text-white">Editar Sala: {{ $sala->nombre }} (Sala {{ $sala->numero }})</h2>
        </div>

        <form action="{{ route('salas.update', $sala->id) }}" method="POST" class="p-6 space-y-6 bg-white">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="sucursal_id" id="sucursal_id" 
                    ...
                    @foreach($sucursales as $sucursal)
                        <option value="{{ $sucursal->id }}" {{ (old('sucursal_id', $sala->sucursal_id) == $sucursal->id) ? 'selected' : '' }}>
                            {{ $sucursal->nombre }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    ...
                    <select name="nombre" id="nombre" onchange="updateSalaInfo()" 
                        ...
                        <option value="Tradicional" {{ (old('nombre', $sala->nombre) == 'Tradicional') ? 'selected' : '' }}>Tradicional</option>
                        <option value="3D" {{ (old('nombre', $sala->nombre) == '3D') ? 'selected' : '' }}>3D</option>
                        <option value="4D" {{ (old('nombre', $sala->nombre) == '4D') ? 'selected' : '' }}>4D</option>
                        <option value="IMAX" {{ (old('nombre', $sala->nombre) == 'IMAX') ? 'selected' : '' }}>IMAX</option>
                    </select>
                    @error('nombre')
                        <p class="text-red-600 text-xs mt-1 font-bold">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" id="numero_display" value="{{ $sala->numero }}" 
                ...
                <div>
                    ...
                    <input type="text" id="capacidad_display" value="{{ $sala->capacidad }}" 
                        class="block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 bg-gray-100 text-gray-600 font-bold cursor-not-allowed" readonly>
                </div>
            </div>
            ...
            <div>
                ...
                <select name="estatus" id="estatus" 
                    ...
                    <option value="Disponible" {{ (old('estatus', $sala->estatus) == 'Disponible') ? 'selected' : '' }}>🟢 Disponible</option>
                    <option value="Fuera de servicio" {{ (old('estatus', $sala->estatus) == 'Fuera de servicio') ? 'selected' : '' }}>🔴 Fuera de servicio (Mantenimiento)</option>
                </select>
                @error('estatus')
                    <p class="text-red-600 text-xs mt-1 font-bold">{{ $message }}</p>
                @enderror
...
<script>
// La misma función mágica para el Número y los Asientos
...
// Ejecutar al cargar por si modificamos el tipo de sala
